...
public final class StepChanger implements ClientTickEvents.EndTick {
    ...
    private Text buildStatusMessage() {
        MutableText prefix = StepUpTexts.empty()
        ...
        switch (autoJumpState) {
            case StepUpMode.STEP_UP:
                message = (MutableText) StepUpTexts.translatable("mod.stepup.enabled").formatted(Formatting.GREEN);
                break;
            ...
            default:
                message = (MutableText) StepUpTexts.translatable("mod.stepup.disabled").formatted(Formatting.RED);
                break;
        }
